...
def generate_embeddings(texts, model_name=EMBEDDING_MODEL):
    ...
    try:
        # Import here to avoid issues when embeddings are disabled
        import vertexai
        from vertexai.language_models import TextEmbeddingModel
        
        # Initialize Vertex AI
        vertexai.init(project=GCP_PROJECT_ID, location=EMBEDDING_LOCATION)
        
        # Initialize the model
        model = TextEmbeddingModel.from_pretrained(model_name)
        
        # Generate embeddings in batches (API allows max 5 at a time)
        embeddings = []
        for i in range(0, len(texts), EMBEDDING_BATCH_SIZE):
            ...
            batch_embeddings = model.get_embeddings(batch_truncated)
...
def create_chunk_document(file_path, chunk_num, content, metadata, speaker="Unknown"):
    ...
    summary = f"Earnings call {metadata.get('quarter', 'Q?')} {metadata.get('year', '?')} - {speaker}: {content[:250]}..."
...
def extract_metadata_from_filename(filename):
    ...
    metadata = {
        ...
        "company": "Unknown",
        "year": 0,
        "quarter": 0,  # Changed to numeric
        "quarter_str": "Unknown",  # Keep string version for display
        "call_date": "Unknown"
    }
...
def chunk_transcripts(local_file_paths):
    """Chunks transcripts, generates embeddings, saves to disk, and converts to Document objects for import."""
